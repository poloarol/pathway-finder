...
const MouseEventHandler = Class.extend({
  /** **init**
  ...
  handleClick(chart) {
    ...
    if (clicked != undefined && clicked.onClick != undefined) onClick = clicked.onClick;
    else if (clicked && clicked.parent && clicked.parent.onClick) onClick = clicked.parent.onClick;
  ...
  handleMouseover(chart) {
    ...
    if (clicked && clicked.tooltips.length > 0) clicked.fireTooltips();
  },
  ...
  /** **handleMouseStyle**
    ...
    * @api internal
    */
  handleMouseStyle(chart) {
    const me = chart.myMouseEventHandler;
    const obj = me.eventElement;
    const { ctx } = chart;

    if (obj && obj.onClick != undefined) ctx.canvas.style.cursor = 'pointer';
    else if (obj && obj.parent && obj.parent.onClick != undefined) ctx.canvas.style.cursor = 'pointer';
    else ctx.canvas.style.cursor = 'auto';
  },
...
CanvasRenderingContext2D.prototype.isPointInPath_mozilla = function (x, y) {
  if (navigator.userAgent.indexOf('Firefox') != -1) {
    ...
    this.restore();
  } else var ret = this.isPointInPath(x, y);
