...
const Tooltip = Class.extend({
  /** **init**
    ...
    * @api internal
    */
  init(text, placement, verticalOffset, opts) {
	   const tt = this;
	   tt.text = text;
    tt.placement = placement || 'above';
    tt.verticalOffset = verticalOffset || 0;
    // set option attributes if any
    for (const attribute in opts) tt[attribute] = opts[attribute];
  ...
  draw(feature, opacity) {
    ...
    // define attributes
    const { roundness } = this.chart.tooltips;
    const { font } = this.chart.tooltips.text;
    ...
    let y;
    if (this.placement == 'below') y = feature.getPixelPositionY() + feature.getHeight() - this.verticalOffset;
    else y = feature.getPixelPositionY() - height - this.verticalOffset;
    ...
    if (length + x > this.chart.width) x = this.chart.width - length;
    ...
    if (this.chart.tooltips.style == 'light') {
      ...
      fillStyle.addColorStop(0.75, 'rgb(253, 248, 196)');
