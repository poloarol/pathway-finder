...
const Seq = Glyph.extend({
  /** **init**
       ...
       * @api public
       */
  init(type, position, length, seq, opts) {
		   this.seq = seq;
		   this.insertions = [];
  ...
  _draw(ctx, length, height) {
    ...
    if (seq.lane.chart.ntsToPixels() <= seq.fractionLevel) fraction = this.fraction;
    ...
    if (seq.lane.chart.ntsToPixels() <= seq.fractionLevel) ctx.strokeStyle = 'rgba(0,0,0,1)';
