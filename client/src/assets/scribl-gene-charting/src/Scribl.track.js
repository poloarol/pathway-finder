...
const Track = Class.extend({
  ...
  addFeature(feature) {
    ...
    if (new_lane) curr_lane = this.addLane();
  ...
  /** **getDrawStyle**
    ...
    */
  getDrawStyle() {
    if (this.drawStyle) return this.drawStyle;
  ...
  getHeight() {
    ...
    const { laneBuffer } = this.chart;
    const drawStyle = this.getDrawStyle();
    if (drawStyle == 'line' || drawStyle == 'collapse') numLanes = 1;
  ...
  getPixelPositionY() {
    ...
    if (!track.chart.scale.off) y = track.chart.getScaleHeight() + track.chart.laneBuffer;
    else y = 0;
  ...
  calcCoverageData() {
    const { lanes } = this;
    const { min } = this.chart.scale;
    const { max } = this.chart.scale;
  ...
  draw() {
    ...
    const { lanes } = track;
    const { laneBuffer } = track.chart;
    const { trackBuffer } = track.chart;
    let y = laneSize + trackBuffer;
    const { ctx } = track.chart;
    ...
    if (!dontDraw) {
      ...
      if (style == undefined || style == 'expand') {
        ...
        if (lanes.length > 0) ctx.translate(0, lanes[0].getHeight() + laneBuffer);
