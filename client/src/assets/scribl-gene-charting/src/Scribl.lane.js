...
const Lane = Class.extend({
  ...
  addFeature(feature) {
    ...
    if (feature.length + feature.position > this.chart.scale.max || !this.chart.scale.max) this.chart.scale.max = feature.length + feature.position;
    if (feature.position < this.chart.scale.min || !this.chart.scale.min) this.chart.scale.min = feature.position;
  ...
  loadFeatures(features) {
    ...
    for (let i = 0; i < featureNum; i++) this.addFeature(features[i]);
  },
  ...
  /** **draw**
    ...
    */
  draw() {
	   const { min } = this.track.chart.scale;
    const { max } = this.track.chart.scale;
	   let hasGlyphs = false;
  ...
  filterFeaturesByPosition(start, end) {
    ...
    for (let i = 0; i < numFeatures; i++) {
      ...
      if ((ftStart >= start && ftStart <= end) || (ftEnd >= start && ftEnd <= end)) features.push(lane.features[i]);
    }
