...
const Glyph = Class.extend({
  /** **init**
    ...
    * @api internal
    */
  init(type, pos, length, strand, opts) {
    const glyph = this;
  ...
  getPixelPositionX() {
    ...
    if (glyph.parent) var position = glyph.position + glyph.parent.position - glyph.lane.track.chart.scale.min;
    else var position = glyph.position - glyph.lane.track.chart.scale.min;
  ...
  drawText(text) {
    ...
    if (align == 'start') {
      if (glyph.strand == '+') align = 'left';
      else align = 'right';
    } else if (align == 'end') {
      if (glyph.strand == '+') align = 'right';
      else align = 'left';
    }

    // handle absolute text alignment
    ctx.textAlign = align;
    if (align == 'left') placement = 0 + padding;
    else if (align == 'center') placement = length / 2;
    else if (align == 'right') placement = length - padding;
    ...
    if (text && text != '') {
      ...
      if (glyph.glyphType == 'Complex') {
        ...
        if (align == 'center') offset = -(ctx.measureText(text).width / 2 + padding / 2);
  ...
  getFillStyle() {
    ...
    if (color instanceof Array) {
      ...
      let currColor;
      for (let i = 0; currColor = color[i], i < color.length; i++) lineargradient.addColorStop(i / (color.length - 1), currColor);
  ...
  /** **fireTooltips**
     ...
     */
  fireTooltips() {
    for (let i = 0; i < this.tooltips.length; i++) this.tooltips[i].fire();
  },
  ...
  draw() {
    ...
    const { fillStyle } = glyph.ctx;
    const position = glyph.getPixelPositionX();
    const height = glyph.getHeight();

    (height < fontSizeMin) ? glyph.ctx.font = `${fontSizeMin}px ${font}` : glyph.ctx.font = `${height * 0.9}px ${font}`;

    // setup ctx position and orientation
    glyph.ctx.translate(position, 0);
    if (glyph.strand == '-' && !glyph.isSubFeature()) glyph.ctx.transform(-1, 0, 0, 1, glyph.getPixelLength(), 0);
