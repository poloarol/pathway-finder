/* -*- mode: javascript; c-basic-offset: 4; indent-tabs-mode: nil -*- */

//
// Dalliance Genome Explorer
// (c) Thomas Down 2006-2011
//
// bam.js: indexed binary alignments
//

const BAM_MAGIC = 21840194;
const BAI_MAGIC = 21578050;

function BamFile() {
}

function Vob(b, o) {
  this.block = b;
  this.offset = o;
}

Vob.prototype.toString = function () {
  return `${this.block}:${this.offset}`;
};

function Chunk(minv, maxv) {
  this.minv = minv; this.maxv = maxv;
}

function makeBam(data, bai, callback) {
  const bam = new BamFile();
  bam.data = data;
  bam.bai = bai;

  bam.data.fetch((r) => {
    if (!r) {
      return alert("Couldn't access BAM");
    }

    const unc = unbgzf(r);
    const uncba = new Uint8Array(unc);

    const magic = readInt(uncba, 0);
    const headLen = readInt(uncba, 4);
    let header = '';
    for (var i = 0; i < headLen; ++i) {
      header += String.fromCharCode(uncba[i + 8]);
    }

    const nRef = readInt(uncba, headLen + 8);
    let p = headLen + 12;

    bam.chrToIndex = {};
    bam.indexToChr = [];
    for (var i = 0; i < nRef; ++i) {
      const lName = readInt(uncba, p);
      let name = '';
      for (let j = 0; j < lName - 1; ++j) {
        name += String.fromCharCode(uncba[p + 4 + j]);
      }
      const lRef = readInt(uncba, p + lName + 4);
      // dlog(name + ': ' + lRef);
      bam.chrToIndex[name] = i;
      if (name.indexOf('chr') == 0) {
        bam.chrToIndex[name.substring(3)] = i;
      } else {
        bam.chrToIndex[`chr${name}`] = i;
      }
      bam.indexToChr.push(name);

      p = p + 8 + lName;
    }

    if (bam.indices) {
      return callback(bam);
    }
  });

  bam.bai.fetch((header) => { // Do we really need to fetch the whole thing? :-(
    if (!header) {
      return alert("Couldn't access BAI");
    }

    const uncba = new Uint8Array(header);
    const baiMagic = readInt(uncba, 0);
    if (baiMagic != BAI_MAGIC) {
      return alert('Not a BAI file');
    }

    const nref = readInt(uncba, 4);

    bam.indices = [];

    let p = 8;
    for (let ref = 0; ref < nref; ++ref) {
      const blockStart = p;
      const nbin = readInt(uncba, p); p += 4;
      for (let b = 0; b < nbin; ++b) {
        const bin = readInt(uncba, p);
        const nchnk = readInt(uncba, p + 4);
        p += 8 + (nchnk * 16);
      }
      const nintv = readInt(uncba, p); p += 4;
      p += (nintv * 8);
      if (nbin > 0) {
        bam.indices[ref] = new Uint8Array(header, blockStart, p - blockStart);
      }
    }
    if (bam.chrToIndex) {
      return callback(bam);
    }
  });
}


BamFile.prototype.blocksForRange = function (refId, min, max) {
  const index = this.indices[refId];
  if (!index) {
    return [];
  }

  const intBinsL = reg2bins(min, max);
  const intBins = [];
  for (var i = 0; i < intBinsL.length; ++i) {
    intBins[intBinsL[i]] = true;
  }
  const leafChunks = []; let
    otherChunks = [];

  const nbin = readInt(index, 0);
  let p = 4;
  for (let b = 0; b < nbin; ++b) {
    const bin = readInt(index, p);
    const nchnk = readInt(index, p + 4);
    // dlog('bin=' + bin + '; nchnk=' + nchnk);
    p += 8;
    if (intBins[bin]) {
      for (let c = 0; c < nchnk; ++c) {
        const cs = readVob(index, p);
        const ce = readVob(index, p + 8);
        (bin < 4681 ? otherChunks : leafChunks).push(new Chunk(cs, ce));
        p += 16;
      }
    } else {
      p += (nchnk * 16);
    }
  }
  //    dlog('leafChunks = ' + miniJSONify(leafChunks));
  //    dlog('otherChunks = ' + miniJSONify(otherChunks));

  const nintv = readInt(index, p);
  let lowest = null;
  const minLin = Math.min(min >> 14, nintv - 1); const
    maxLin = Math.min(max >> 14, nintv - 1);
  for (var i = minLin; i <= maxLin; ++i) {
    const lb = readVob(index, p + 4 + (i * 8));
    if (!lb) {
      continue;
    }
    if (!lowest || lb.block < lowest.block || lb.offset < lowest.offset) {
      lowest = lb;
    }
  }
  //     dlog('Lowest LB = ' + lowest);

  const prunedOtherChunks = [];
  if (lowest != null) {
    for (var i = 0; i < otherChunks.length; ++i) {
      const chnk = otherChunks[i];
      if (chnk.maxv.block >= lowest.block && chnk.maxv.offset >= lowest.offset) {
        prunedOtherChunks.push(chnk);
      }
    }
  }
  //     dlog('prunedOtherChunks = ' + miniJSONify(prunedOtherChunks));
  otherChunks = prunedOtherChunks;

  const intChunks = [];
  for (var i = 0; i < otherChunks.length; ++i) {
    intChunks.push(otherChunks[i]);
  }
  for (var i = 0; i < leafChunks.length; ++i) {
    intChunks.push(leafChunks[i]);
  }

  intChunks.sort((c0, c1) => {
    const dif = c0.minv.block - c1.minv.block;
    if (dif != 0) {
      return dif;
    }
    return c0.minv.offset - c1.minv.offset;
  });
  const mergedChunks = [];
  if (intChunks.length > 0) {
    let cur = intChunks[0];
    for (var i = 1; i < intChunks.length; ++i) {
      const nc = intChunks[i];
      if (nc.minv.block == cur.maxv.block /* && nc.minv.offset == cur.maxv.offset */) { // no point splitting mid-block
        cur = new Chunk(cur.minv, nc.maxv);
      } else {
        mergedChunks.push(cur);
        cur = nc;
      }
    }
    mergedChunks.push(cur);
  }
  //    dlog('mergedChunks = ' + miniJSONify(mergedChunks));

  return mergedChunks;
};

BamFile.prototype.fetch = function (chr, min, max, callback) {
  const thisB = this;

  const chrId = this.chrToIndex[chr];
  let chunks;
  if (chrId === undefined) {
    chunks = [];
  } else {
    chunks = this.blocksForRange(chrId, min, max);
    if (!chunks) {
      callback(null, 'Error in index fetch');
    }
  }

  const records = [];
  let index = 0;
  let data;

  function tramp() {
    if (index >= chunks.length) {
      return callback(records);
    } if (!data) {
      //             dlog('fetching ' + index);
      const c = chunks[index];
      const fetchMin = c.minv.block;
      const fetchMax = c.maxv.block + (1 << 16); // *sigh*
      thisB.data.slice(fetchMin, fetchMax - fetchMin).fetch((r) => {
        data = unbgzf(r, c.maxv.block - c.minv.block + 1);
        return tramp();
      });
    } else {
      const ba = new Uint8Array(data);
      thisB.readBamRecords(ba, chunks[index].minv.offset, records, min, max, chrId);
      data = null;
      ++index;
      return tramp();
    }
  }
  tramp();
};

const SEQRET_DECODER = ['=', 'A', 'C', 'x', 'G', 'x', 'x', 'x', 'T', 'x', 'x', 'x', 'x', 'x', 'x', 'N'];
const CIGAR_DECODER = ['M', 'I', 'D', 'N', 'S', 'H', 'P', '=', 'X', '?', '?', '?', '?', '?', '?', '?'];

function BamRecord() {
}

BamFile.prototype.readBamRecords = function (ba, offset, sink, min, max, chrId) {
  while (true) {
    const blockSize = readInt(ba, offset);
    const blockEnd = offset + blockSize + 4;
    if (blockEnd >= ba.length) {
      return sink;
    }

    const record = new BamRecord();

    const refID = readInt(ba, offset + 4);
    const pos = readInt(ba, offset + 8);

    const bmn = readInt(ba, offset + 12);
    const bin = (bmn & 0xffff0000) >> 16;
    const mq = (bmn & 0xff00) >> 8;
    const nl = bmn & 0xff;

    const flag_nc = readInt(ba, offset + 16);
    const flag = (flag_nc & 0xffff0000) >> 16;
    const nc = flag_nc & 0xffff;

    const lseq = readInt(ba, offset + 20);

    const nextRef = readInt(ba, offset + 24);
    const nextPos = readInt(ba, offset + 28);

    const tlen = readInt(ba, offset + 32);

    let readName = '';
    for (var j = 0; j < nl - 1; ++j) {
      readName += String.fromCharCode(ba[offset + 36 + j]);
    }

    let p = offset + 36 + nl;

    let lengthOnRef = 0;
    let cigar = '';
    for (let c = 0; c < nc; ++c) {
      const cigop = readInt(ba, p);
      const op = (cigop >> 4);
      const opLtr = CIGAR_DECODER[cigop & 0xf];
      if (opLtr == 'M' || opLtr == 'I' || opLtr == 'X') lengthOnRef += op;
      cigar = cigar + op + opLtr;
      p += 4;
    }
    record.cigar = cigar;
    record.lengthOnRef = lengthOnRef;

    let seq = '';
    const seqBytes = (lseq + 1) >> 1;
    for (var j = 0; j < seqBytes; ++j) {
      const sb = ba[p + j];
      seq += SEQRET_DECODER[(sb & 0xf0) >> 4];
      seq += SEQRET_DECODER[(sb & 0x0f)];
    }
    p += seqBytes;
    record.seq = seq;

    let qseq = '';
    for (var j = 0; j < lseq; ++j) {
      qseq += String.fromCharCode(ba[p + j]);
    }
    p += lseq;
    record.quals = qseq;

    record.pos = pos;
    record.mq = mq;
    record.readName = readName;
    record.segment = this.indexToChr[refID];

    while (p < blockEnd) {
      const tag = String.fromCharCode(ba[p]) + String.fromCharCode(ba[p + 1]);
      const type = String.fromCharCode(ba[p + 2]);
      var value;

      if (type == 'A') {
        value = String.fromCharCode(ba[p + 3]);
        p += 4;
      } else if (type == 'i' || type == 'I') {
        value = readInt(ba, p + 3);
        p += 7;
      } else if (type == 'c' || type == 'C') {
        value = ba[p + 3];
        p += 4;
      } else if (type == 's' || type == 'S') {
        value = readShort(ba, p + 3);
        p += 5;
      } else if (type == 'f') {
        throw 'FIXME need floats';
      } else if (type == 'Z') {
        p += 3;
        value = '';
        for (;;) {
          const cc = ba[p++];
          if (cc == 0) {
            break;
          } else {
            value += String.fromCharCode(cc);
          }
        }
      } else {
        throw `Unknown type ${type}`;
      }
      record[tag] = value;
    }

    if (!min || record.pos <= max && record.pos + lseq >= min) {
      if (chrId === undefined || refID == chrId) {
        sink.push(record);
      }
    }
    offset = blockEnd;
  }

  // Exits via top of loop.
};

function readInt(ba, offset) {
  return (ba[offset + 3] << 24) | (ba[offset + 2] << 16) | (ba[offset + 1] << 8) | (ba[offset]);
}

function readShort(ba, offset) {
  return (ba[offset + 1] << 8) | (ba[offset]);
}

function readVob(ba, offset) {
  const block = ((ba[offset + 6] & 0xff) * 0x100000000) + ((ba[offset + 5] & 0xff) * 0x1000000) + ((ba[offset + 4] & 0xff) * 0x10000) + ((ba[offset + 3] & 0xff) * 0x100) + ((ba[offset + 2] & 0xff));
  const bint = (ba[offset + 1] << 8) | (ba[offset]);
  if (block == 0 && bint == 0) {
    return null; // Should only happen in the linear index?
  }
  return new Vob(block, bint);
}

function unbgzf(data, lim) {
  lim = Math.min(lim || 1, data.byteLength - 100);
  const oBlockList = [];
  const ptr = [0];
  let totalSize = 0;

  while (ptr[0] < lim) {
    const ba = new Uint8Array(data, ptr[0], 100); // FIXME is this enough for all credible BGZF block headers?
    const xlen = (ba[11] << 8) | (ba[10]);
    // dlog('xlen[' + (ptr[0]) +']=' + xlen);
    const unc = jszlib_inflate_buffer(data, 12 + xlen + ptr[0], Math.min(65536, data.byteLength - 12 - xlen - ptr[0]), ptr);
    ptr[0] += 8;
    totalSize += unc.byteLength;
    oBlockList.push(unc);
  }

  if (oBlockList.length == 1) {
    return oBlockList[0];
  }
  const out = new Uint8Array(totalSize);
  let cursor = 0;
  for (let i = 0; i < oBlockList.length; ++i) {
    const b = new Uint8Array(oBlockList[i]);
    arrayCopy(b, 0, out, cursor, b.length);
    cursor += b.length;
  }
  return out.buffer;
}

//
// Binning (transliterated from SAM1.3 spec)
//

/* calculate bin given an alignment covering [beg,end) (zero-based, half-close-half-open) */
function reg2bin(beg, end) {
  --end;
  if (beg >> 14 == end >> 14) return ((1 << 15) - 1) / 7 + (beg >> 14);
  if (beg >> 17 == end >> 17) return ((1 << 12) - 1) / 7 + (beg >> 17);
  if (beg >> 20 == end >> 20) return ((1 << 9) - 1) / 7 + (beg >> 20);
  if (beg >> 23 == end >> 23) return ((1 << 6) - 1) / 7 + (beg >> 23);
  if (beg >> 26 == end >> 26) return ((1 << 3) - 1) / 7 + (beg >> 26);
  return 0;
}

/* calculate the list of bins that may overlap with region [beg,end) (zero-based) */
const MAX_BIN = (((1 << 18) - 1) / 7);
function reg2bins(beg, end) {
  const i = 0; let k; const
    list = [];
  --end;
  list.push(0);
  for (k = 1 + (beg >> 26); k <= 1 + (end >> 26); ++k) list.push(k);
  for (k = 9 + (beg >> 23); k <= 9 + (end >> 23); ++k) list.push(k);
  for (k = 73 + (beg >> 20); k <= 73 + (end >> 20); ++k) list.push(k);
  for (k = 585 + (beg >> 17); k <= 585 + (end >> 17); ++k) list.push(k);
  for (k = 4681 + (beg >> 14); k <= 4681 + (end >> 14); ++k) list.push(k);
  return list;
}
